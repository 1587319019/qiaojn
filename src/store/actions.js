...
export const updateApplyList = function({ commit, state }, data) {
	...
	let applyList = isGroup
		? JSON.parse(JSON.stringify(state.groupApplyList))
		: JSON.parse(JSON.stringify(state.friendApplyList));
...
export const initApplyList = function({ commit, state }, isGroup) {
	let url = '';
	isGroup ? (url = 'getFriendApplyList') : (url = 'getFriendApplyList');
	Common.axios({
		url: url
	}).then((res) => {
		let list = [];
