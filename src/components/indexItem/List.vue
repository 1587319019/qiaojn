    ...
    <div class="list">
        <el-tabs :stretch="true" class="el-tabs" @tab-click="changeType">
            <el-tab-pane label="好友">
                <ul>
                    <li
                        @click="selectSession(item, false)"
                        class="list-li"
                        v-for="(item, index) in friendList"
                        :key="index"
                        :class="{
                            active: item.userId === currentFriend.friendId
                        }"
                    >
                        <info-card
                            :userId="item.userId"
                        ...
                        <p class="name">
                            {{ item | nameText
                                ...
                                class="status el-icon-circle-check"
...
export default {
    ...
    methods: {
        ...mapActions(["initGroupFriendList"]),
        ...
        getGroupUserList: function(groupId) {
            var that = this;
            let hasGroupFriend = that.$store.getters.hasGroupFriendList(
                groupId
            );
            if (hasGroupFriend) {
                return false;
    ...
    created() {
        ...
        Common.axios({
            ...
            res => {
                //let userList = res.data.friendList || [];
                let userList = [
                    {
                        userId: '111111',
                        username: '测试1'
